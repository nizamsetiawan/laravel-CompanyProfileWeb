...
class AdminController extends Controller
{
    ...
    // ormawa
    public function ormawaAddShow()
    {
        return view('admin.ormawa.add');
    }
    public function ormawaStore(Request $request)
    {

        $ormawa = new Ormawa();
        $ormawa->nama =  $request->get('nama');
        $ormawa->nama_singkatan =  $request->get('nama_singkatan');
        $ormawa->slug = Str::slug($request->get('nama'));
        $ormawa->nama_instagram = $request->get('nama_instagram');
        $ormawa->oa_line = $request->get('oa_line');
        $ormawa->deskripsi = $request->get('deskripsi');

        $request->validate([
            'link_foto' => 'mimes:jpeg,png',
        ]);
        if (request()->hasFile('link_foto')){
            $image_name = $request->get('nama_singkatan').'.png';
            $link_ormawa = 'bemft_assets/images/ormawa';
            $link_db = 'bemft_assets/images/ormawa/'.$request->get('nama_singkatan').'.png';
            $ormawa->link_foto = $link_db;

            $ormawa->save();

            $request->file('link_foto')->move($link_ormawa, $image_name);
        }


        $ormawa->save();
        return redirect()->back()->with(['success' => 'Berhasil menambahkan ormawa']);
    }
    public function ormawaEditShow(){
        $ormawas = Ormawa::latest()->paginate(6);
        return view('admin.ormawa.show', compact('ormawas'));
    }
    public function ormawaEdit($slug){
        $ormawas = Ormawa::where('slug',$slug)->get();
        return view('admin.ormawa.edit', compact('ormawas'));
    }
    public function ormawaEditUpdate(Request $request){
        $ormawa = Ormawa::find($request->get('ormawa_id'));
        $ormawa->nama =  $request->get('nama_ormawa');
        $ormawa->nama_singkatan =  $request->get('nama_ormawa_singkatan');
        $ormawa->slug = Str::slug($request->get('nama_ormawa'));
        $ormawa->deskripsi = $request->get('deskripsi');
        $ormawa->save();
        return redirect()->route('ormawa.show')->with(['success' => 'Berhasil melakukan update pada ormawa']);
    }
